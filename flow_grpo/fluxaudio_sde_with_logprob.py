...
def sde_step_with_logprob(
    ...
    step_index = [index_for_timestep(t, timesteps) for t in timestep]
    prev_step_index = [step+1 for step in step_index]
    step_index = torch.tensor(step_index, device=timesteps.device, dtype=torch.long)
    prev_step_index = torch.tensor(prev_step_index, device=timesteps.device, dtype=torch.long)
    sigma = timesteps[step_index].view(-1, *([1] * (len(sample.shape) - 1)))
    sigma_prev = timesteps[prev_step_index].view(-1, *([1] * (len(sample.shape) - 1)))
    sigma_max = timesteps[1].item()

    dt = sigma_prev - sigma

    if sde_type == 'sde':
        std_dev_t = torch.sqrt(sigma / (1 - torch.where(sigma == 1, sigma_max, sigma)))*noise_level  # sigma_t
        # our sde
