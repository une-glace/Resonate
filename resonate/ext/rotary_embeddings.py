...
def compute_rope_rotations(length: int,
    ...
    with torch.amp.autocast(device_type='cuda', enabled=False):
        ...
        rot = torch.einsum('..., f -> ... f', pos, freqs)  # rot = pos[:, None] * freqs[None, :] -> [seq len, dim]
...
def apply_rope(x: Tensor, rot: Tensor) -> tuple[Tensor, Tensor]:
    with torch.amp.autocast(device_type='cuda', enabled=False):
        seq_len = x.shape[-2]
        rot_len = rot.shape[-4]
        if seq_len != rot_len:  # crop the rope matrix 
            raise ValueError(f"Sequence length {seq_len} does not match RoPE rotation length {rot_len}. ")
...
if __name__ == '__main__': 
    ...
    num_heads = 7
    base_freq = 1
    latent_rot = compute_rope_rotations(latent_seq_len,
                                        hidden_dim // num_heads,
                                        10000,
                                        freq_scaling=base_freq)
    print(latent_rot.shape) 
    ...
    num_heads = 7
    base_freq = 1
    latent_rot_2 = compute_rope_rotations(latent_seq_len,
                                        hidden_dim // num_heads,
                                        10000,
                                        freq_scaling=base_freq)
    print(latent_rot_2.shape) 
    print(torch.all(latent_rot_2[:, :312, :] == latent_rot))
