...
from resonate.ext.rotary_embeddings import apply_rope
...
class TransformerBlock(nn.Module):
    # basic transformer block without AdaLN modulation
    def __init__(self,
                 dim: int,
                 nhead: int,
                 mlp_ratio: float = 4.0,
    ...
    def pre_attention(self, x: torch.Tensor, rot: Optional[torch.Tensor]):
        """get qkv from x and modulation coefficients from condition"""
        # x: BS * N * D
        # cond: BS * D
        x = self.norm1(x)
    ...
    def post_attention(self, x: torch.Tensor, attn_out: torch.Tensor):
        x = x + self.linear1(attn_out)
        r = self.norm2(x)
        x = x + self.ffn(r)
        return x

    def forward(self, x: torch.Tensor, rot: Optional[torch.Tensor]) -> torch.Tensor:
        # x: BS * N * D
        x_qkv = self.pre_attention(x, rot)
        attn_out = attention(*x_qkv)
...
class VanillaFinalBlock(nn.Module):
